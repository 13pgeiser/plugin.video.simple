...
def play2():
  request = urllib2.Request('https://6play-users.6play.fr/v2/platforms/m6group_web/services/rtlhu_rtl_most/users/94b5cafc46ad4fc494b6bd5d9a2dfa66/videos/clip_11903173/upfront-token')
...
def play3():
  ...
  match = re.compile('data-drm-session-key="(.+?)"', re.DOTALL).findall(content)

  payload='key=' + match[0]

  request = urllib2.Request('https://www.filmfriend.de/expressplay/license/license/?type=widevine', headers={'Cookie':cookie}, data=payload)
  ...
  play_item.setProperty('inputstream.adaptive.license_type', 'com.widevine.alpha')
  play_item.setProperty('inputstream.adaptive.license_key', content+'|' + headers + '|R{SSM}|')
  play_item.setProperty('inputstream.adaptive.manifest_type', 'mpd')
  play_item.setProperty('inputstreamaddon', 'inputstream.adaptive')
  play_item.setMimeType('application/dash+xml')
  play_item.setContentLookup(False)
...
if __name__ == '__main__':
  ...
  if params and params['action'] == 'play':
    ...
    list_item = xbmcgui.ListItem(label='FF')
    list_item.setProperty('IsPlayable', 'true')
    list_item.setInfo('video', {'mediatype': 'video'})
